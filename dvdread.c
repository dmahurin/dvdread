...
struct vts_file_extent_s {
    ...
    unsigned int vtsn;
    unsigned int vpart;
    unsigned int vstart; // start of title vts or menu vts
};
...
static int addfile(char *fname, int start, int len) {
    ...
    if (type != 0) {
        ...
        } else if (2 == sscanf(bname, "VTS_%d_%d", &vtsn, &vpart)) {
            type += 3;
            if(vpart > 1) {
                int n = nfiles;
                while(--n > 0) {
                    if(file[n].vtsn == vtsn && file[n].vpart == 1) {
                        file[nfiles].vstart = file[n].start;
                        break;
                    }
                }
            }
    ...
    file[nfiles].vstart = vstart;
    file[nfiles].vtsn = vtsn;
    file[nfiles++].end = end;
    return 1;
}
...
int find_next_vobu(dvd_reader_t *dvd, int vtsn, int vpart, int vstart, int s) {
    s -= vstart;

    ifo_handle_t *vts_ifo = ifoOpen(dvd, vtsn);

    vobu_admap_t *vobu_admap = vpart == 0 ? vts_ifo->menu_vobu_admap : vts_ifo->vts_vobu_admap;
    int n = (vobu_admap->last_byte + 1 + - VOBU_ADMAP_SIZE) / 4;

    for (int i = 0; i < n; i++) {
        if(s < vobu_admap->vobu_start_sectors[i]) {
		s = vobu_admap->vobu_start_sectors[i];
		break;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    for (s = s1, ss = s1, lastfile = -1, curfile = -1; s < s2;
        ...
        if(skip_to_s > s) { r = s; }
        else if (curfile >= 0 && (file[curfile].type == vmg_vob
        ...
        if (r != 1) {
            ...
            if(file[curfile].type == vts_vob) {
                skip_to_s = find_next_vobu(prdr, file[curfile].vtsn, file[curfile].vpart, file[curfile].vstart, s);
                ss_fprintf(&ss, s, file, curfile, 1, stderr, "skiping to vobu at %d\n", skip_to_s);
            }
        } else {
